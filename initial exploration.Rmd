...
forestpalette<-pale <- colorFactor(palette = "viridis",
...
leaflet() %>%
  ...
  addLegend(pal = forestpalette, values = aggforests$Sheet1__Na)%>%
  addMarkers(data = group_campsite, popup = "Group Campsite") %>%
  addMarkers(data = remote_campsite, popup = "Remote Campsite")
...
forestpalette<-pale <- colorFactor(palette = "inferno",
                    domain = aggforests$Sheet1__Na)

leaflet()%>%
  addTiles()%>%
  addPolygons(data=aggforests, color = ~forestpalette(aggforests$Sheet1__Na))
